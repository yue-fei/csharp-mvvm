...
using WPF.MVVM.Core;
using WPF.MVVM.Example.ViewModel;
...
namespace WPF.MVVM.Example
{
    public partial class ViewModelLocator
    {
        ...
        public ViewModelLocator()
        {
            ...
            if (!ViewModelBase.IsInDesignModeStatic) Init();
        }
    ...
    public partial class ViewModelLocator
    {
        ...
        public MainWindowViewModel MainWindowViewModel
        {
            get
            {
                string key = GetViewModelKey(typeof(MainWindowViewModel), string.Empty);
                if (ViewModels.ContainsKey(key))
                {
                    return (MainWindowViewModel)ViewModels[key];
                }
                else
                {
                    return RegisterViewModel<MainWindowViewModel>();
                }
            }
        }

        public FirstWindowViewModel FirstWindowViewModel
        {
            get
            {
                string key = GetViewModelKey(typeof(FirstWindowViewModel), string.Empty);
                if (ViewModels.ContainsKey(key))
                {
                    return (FirstWindowViewModel)ViewModels[key];
                }
                else
                {
                    return RegisterViewModel<FirstWindowViewModel>();
                }
            }
        }

        public SecondWindowViewModel SecondWindowViewModel
        {
            get
            {
                string key = GetViewModelKey(typeof(SecondWindowViewModel), string.Empty);
                if (ViewModels.ContainsKey(key))
                {
                    return (SecondWindowViewModel)ViewModels[key];
                }
                else
                {
                    return RegisterViewModel<SecondWindowViewModel>();
                }
            }
        }
    }
