    ...
    public class ViewControlLocator
    {
        ...
        private T CreateUserControl<T>() where T : new()
        {
            ...
            if (t.IsSubclassOf(typeof(UserControl)))
            {
                ...
                if (uc.Content == null)
                {
                    MethodInfo mi = typeof(T).GetTypeInfo().GetDeclaredMethod("InitializeComponent");
                    mi.Invoke(uc, null);
                }
            }
            else if (t.Equals(typeof(Page)))
            {
                Page page = rst as Page;
                if (page.Content == null)
                {
                    MethodInfo mi = typeof(T).GetRuntimeMethod("InitializeComponent", null);
                    mi.Invoke(page, null);
                }
            }
            return rst;
